...
function Card ({id, name, xAntibody}) {
  ...
  return (
    <div className='digi-container w-72 h-92 p-5 m-5 text-center bg-white  rounded-t-md '>
          ...
          <img className='w-full h-60 rounded-t-md ' src={`https://digimon-api.com/images/digimon/w/${name?.replace(/\s+/g, '_')}.png`}/>
          </div>
          <h1 className='mb-5'>{name}</h1>
          <p>{xAntibody}</p>
          {/* <p>{details}</p> */}
          <button >
            <Link className='hover:bg-yellow-400 bg-yellow-300 px-5 py-2 rounded-lg' href="/details">Details</Link>
          </button>
        </div>
  )
}